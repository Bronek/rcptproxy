...
Basically, installation of RcptProxy consists of 5 steps:
1. create destination directory for application files; suggested one is
  ...
  write in this directory; SYSTEM account (i.e. LocalSystem) needs read
  access.
2. copy *.vbs and *.dll files (register.vbs, unregister.vbs, 
  rcptproxy.dll, msvcr71.dll, msvcp71.dll) to the directory created in 1.
  It is recommended that you also copy *.txt files there. Visual C++ 2003
  runtime files (msvcr71.dll and msvcp71.dll) are required only if you 
  do not have these files already in your system  directory (e.g.
  C:\WINDOWS\system32).
...
RcptProxy is so called SMTP event sink, i.e. "plugin" into IIS SMTP
service. In order for IIS to run it, it has to be configured (e.g.
registered) in the metabase. You can perform this registration using
...
values in unregister.vbs script. You will need it to uninstall component,
...
metabase. You can easily edit the metabase using graphical tool 
...
First you need to find the metabase path where component "RcptProxy.Sink"
has been registered (it's inside metabase key named "SinkClass"). This
path should be similar to given below:
...
0 (String) - IP of SMTP server that should verify all incoming RCPT TO 
  commands (internal SMTP server). This is last value you can set in
...
65554 (String) - "HELO" command sent from proxy when establishing
  ...
  requests in log of the other server. It will default  to "world" if not 
  set;
...
65559 (DWORD) - maximum time in milliseconds allowed for the verification
  ...
  seconds);
65560 (MultiString) - list of IP addresses that should bypass RcptProxy. 
...
If you decide to compile project yourself, please be aware of following:
